...
async function readIP(dbRoot, ips) {
  ...
  const CountryDB = new mmdbLib.Reader(files[0]);
  const CityDB = new mmdbLib.Reader(files[1]);
  const ASNDB = new mmdbLib.Reader(files[2]);
...
const isTarAvailable = commandExists.sync("tar");
const edition_id = {
  asn: "GeoLite2-ASN",
  city: "GeoLite2-City",
...
exports.CLI_USERAGENT = CLI_USERAGENT;
