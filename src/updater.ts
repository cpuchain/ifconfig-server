...
export async function downloadSha(
    database: keyof typeof edition_id,
    licenseKey: string,
) {
    const edition = edition_id[database];
    ...
    const dbVersion = parseInt(
        gzFile.replace(`${edition}_`, '').replace('.tar.gz', ''),
    );
...
export async function downloadDB(
    ...
    const { digest, gzFile, dbVersion } = await downloadSha(
        database,
...
export async function updateDB(
    ...
    const { edition, digest, gzFile, dbVersion } = await downloadDB(
        database,
        updater,
    );
...
export async function fetchAsnList() {
    const resp = await (
        await fetch(
            'https://raw.githubusercontent.com/cpuchain/bad-asn-list/master/bad-asn-list.csv',
        )
    ).text();
...
export async function updateAll(updater: Updater) {
    ...
    await fs.writeFile(
        path.join(config.dbRoot, 'torlist.json'),
        JSON.stringify(torList),
    );
    logger.debug(`Updated ${torList.length} exit nodes`);

    const asnList = await fetchAsnList();
    await fs.writeFile(
        path.join(config.dbRoot, 'asnlist.json'),
        JSON.stringify(asnList),
    );
    ...
    const allUpdates = (
        await Promise.all(
            Object.keys(edition_id).map((e) =>
                updateDB(e as keyof typeof edition_id, updater),
            ),
        )
    ).filter((e) => e) as Array<{ dbVersion: number }>;
    ...
    await fs.writeFile(
        path.join(config.dbRoot, 'last_update.txt'),
        String(dbVersion),
    );
...
export async function setupDB(updater: Updater) {
    ...
    await fs.copyFile(
        path.join(process.cwd(), './views/asnlist.json'),
        path.join(config.dbRoot, 'asnlist.json'),
    );
    await fs.copyFile(
        path.join(process.cwd(), './views/torlist.json'),
        path.join(config.dbRoot, 'torlist.json'),
    );

    await fs.copyFile(
        path.join(process.cwd(), './views/GeoLite2-ASN.tar.gz.sha256'),
        path.join(config.dbRoot, 'GeoLite2-ASN.tar.gz.sha256'),
    );
    await fs.copyFile(
        path.join(process.cwd(), './views/GeoLite2-City.tar.gz.sha256'),
        path.join(config.dbRoot, 'GeoLite2-City.tar.gz.sha256'),
    );
    await fs.copyFile(
        path.join(process.cwd(), './views/GeoLite2-Country.tar.gz.sha256'),
        path.join(config.dbRoot, 'GeoLite2-Country.tar.gz.sha256'),
    );
    await fs.copyFile(
        path.join(process.cwd(), './views/last_update.txt'),
        path.join(config.dbRoot, 'last_update.txt'),
    );

    // Get DB version
    const dbVersion = Number(
        await fs.readFile(path.join(process.cwd(), './views/last_update.txt'), {
            encoding: 'utf8',
        }),
    );

    await fs.copyFile(
        path.join(process.cwd(), './views/GeoLite2-ASN.tar.gz'),
        `GeoLite2-ASN_${dbVersion}.tar.gz`,
    );
    await fs.copyFile(
        path.join(process.cwd(), './views/GeoLite2-City.tar.gz'),
        `GeoLite2-City_${dbVersion}.tar.gz`,
    );
    await fs.copyFile(
        path.join(process.cwd(), './views/GeoLite2-Country.tar.gz'),
    ...
    await extractDB(
        'asn',
        `GeoLite2-ASN_${dbVersion}.tar.gz`,
        await readSha('asn', updater),
        updater,
    );
    await extractDB(
        'city',
        `GeoLite2-City_${dbVersion}.tar.gz`,
        await readSha('city', updater),
        updater,
    );
    await extractDB(
        'country',
...
export default class Updater {
    ...
    constructor(config: Config) {
        this.config = config;
        this.logger = LoggerFactory(config, 'Main', 'Updater');
        this.dbVersion = 0;
    }
    ...
    async setup() {
        if (!this.config.licenseKey) {
            this.logger.warning(
                'MaxMind License Key not found, will not update the latest IP DB',
            );
        }
